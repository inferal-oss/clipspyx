...
/*                                                           */
/*      7.00: The + functions returns 0 if given no          */
/*            arguments, otherwise the sum of all arguments  */
/*            is returned.                                   */
/*                                                           */
/*            The - functions returns the negative of its    */
/*            first argument if passed just one argument.    */
/*                                                           */
/*            The * functions returns 1 if given no          */
/*            arguments, otherwise the product of all        */
/*            arguments is returned.                         */
/*                                                           */
/*            The / functions returns the reciprocal of its  */
/*            argument if there is just one argument.        */
/*                                                           */
/*************************************************************/
...
void BasicMathFunctionDefinitions(
  ...
  {
#if ! RUN_TIME
   AddUDF(theEnv,"+","ld",0,UNBOUNDED,"ld",AdditionFunction,"AdditionFunction",NULL);
   AddUDF(theEnv,"*","ld",0,UNBOUNDED,"ld",MultiplicationFunction,"MultiplicationFunction",NULL);
   AddUDF(theEnv,"-","ld",1,UNBOUNDED,"ld",SubtractionFunction,"SubtractionFunction",NULL);
   AddUDF(theEnv,"/","d",1,UNBOUNDED,"ld",DivisionFunction,"DivisionFunction",NULL);
...
void SubtractionFunction(
  ...
  UDFValue *returnValue)
  {
   ...
   /*==========================================================*/
   /* A single argument returns the negative of that argument. */
   /*==========================================================*/
...
void DivisionFunction(
  ...
  UDFValue *returnValue)
  {
   ...
   /*============================================================*/
   /* A single argument returns the reciprical of that argument. */
   /*============================================================*/
   ...
   if (! UDFHasNextArgument(context))
     {
      ...
      returnValue->floatValue = CreateFloat(theEnv,1.0 / ftotal);
      return;
     }
